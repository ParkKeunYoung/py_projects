...
game_txt = [
    '게임 제목을 입력하시오\n',
    '아무것도 입력하지 않았습니다. 다시!!\n',
    '입력하신 "%s" 게임 제목은 최대 28자(현재 "%s"자 입력)를 초과할수 없습니다. 다시 입력하세요..\n'
    ,'v1.0.0 ',
    '게이머의 이름을 입력하세요 : ',
    '이름이 입력되지 않았습니다. 다시~',
    '다시 게임을 할까요?\n',
...
while True: # 조건달성이 안됬을 때 다시하기
    game_title = input( game_txt[0] )

    # 값 체크 
    #   아무것도 입력하지 않으면 => 아무것도 입력하지 않았습니다. 다시!!\n
    if not game_title: # 부정을 잡는 방법 = not. // False 이면 즉, game_title이 [] (빈 공간)이다.
        # game_title이 ''이면, 조건문에 와서는 False로 판단한다
        # '' 은 곧 아무것도 입력하지 않으면이라는 뜻.
        print( game_txt[1] )

    #   20자를 초과하면     => 입력하신 "xx" 게임 제목은 최대 28자(현재 %s자 입력)를 초과할수 없습니다. 다시 입력하세요..\n
    elif len(game_title) > 20 :
        print( game_txt[2] %(game_title,len(game_title)) )

    #   정상입력
    else:
        # 정상 이력을 하면 해당 반복문을 빠져나간다.
        break
print( '[%s]' %game_title)

#############################################################################################
# step 1-2 :
# 게임 프럼프트 출력 (가로 26칸), 한글 제목일 경우 간격 안맞는 경우는 배제하기.
cell_amt = 40
# 포멧팅 사용하는 기호가 서로 다르므로, 다른 케이스로 클자크기를 조절하였다.
form = '={0:^%s}=' %(cell_amt-2)
#==============================
print('='*cell_amt)
#=          게임제목           =
...
#==============================
print('='*cell_amt)
# 26이란 숫자를 40으로 변경되어도 잘 적응되게 코드를 설정
